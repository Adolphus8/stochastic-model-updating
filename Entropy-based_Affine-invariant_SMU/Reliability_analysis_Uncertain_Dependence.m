...
% reliability analysis (under uncertain dependency between epsilon_k and 
% rhoCp) of the slab material accounting for the variability in the thermal 
% properties of the given specimen.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear; clc;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
cc = unifrnd(-1, 1, 1, 1); % Pearson Correlation coefficient;
u = @(x) copularnd('Gaussian', x, Na); % The Gaussian copula

K_model_noisy = @(T,m) (reg_coeff(2,1).*T + reg_coeff(1,1)) + samp_epsilon_k(m); % Generate noisy values of k given temperature T
K_input = normrnd(mean(k_data(:)), std(k_data(:)), Na, 1);                       % Generate the starting seed samples from a Normal distribution
rho_samp = samp_rho(rho_moments(i,:));                                           % Generate the random samples of rhoCp from its SDF distribution
u_samp = u(cc);  % Generate Na samples from the Gaussian compula given a Pearson Correlation coefficient;
[~, i1] = sort(u_samp(:, 1)); [~, i2] = sort(u_samp(:, 2)); 
x1 = zeros(Na, 1); x1(i1) = sort(K_input);  % Correlated samples of k
x2 = zeros(Na, 1); x2(i2) = sort(rho_samp); % Correlated samples of rhoCp

T_dist = zeros(Na,1); 
for j = 1:Na
T_dist(j) = T_model(x1(j), x2(j));
end

K_output = K_model_noisy(T_dist, epsilon_k_moments(i,:)); rho_samp = samp_rho(rho_moments(i,:));
u_samp = u(cc);  % Generate Na samples from the Gaussian compula given a Pearson Correlation coefficient;
[~, i1] = sort(u_samp(:, 1)); [~, i2] = sort(u_samp(:, 2)); 
x1 = zeros(Na, 1); x1(i1) = sort(K_output);  % Correlated samples of k
x2 = zeros(Na, 1); x2(i2) = sort(rho_samp); % Correlated samples of rhoCp

T_dist_new = zeros(Na,1); 
for j = 1:Na
T_dist_new(j) = T_model(x1(j), x2(j));
end
...
if cse == 1
save('Reliability_analysis_CaseI_UncDependence', 'g_out', 'g_pbox', 'epsilon_k_moments', 'rho_moments', 'timeDLMC', 'failure', 'p_rel') 
elseif cse == 2
save('Reliability_analysis_CaseII_UncDependence', 'g_out', 'g_pbox', 'epsilon_k_moments', 'rho_moments', 'timeDLMC', 'failure', 'p_rel')
elseif cse == 3
save('Reliability_analysis_CaseIII_UncDependence', 'g_out', 'g_pbox', 'epsilon_k_moments', 'rho_moments', 'timeDLMC', 'failure', 'p_rel')
end
