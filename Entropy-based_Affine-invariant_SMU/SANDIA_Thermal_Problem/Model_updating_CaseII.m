...
k_temp = [repmat(temperature_data, [1, size(k_data,1)]); [k_data(1,:), k_data(2,:), k_data(3,:), k_data(4,:)]]'; k_temp_corr = corrcoef(k_temp);
sprintf('The Pearson correlation coefficient between Temperature and Thermal conductivity is = %0.5g', k_temp_corr(1,2))

v_temp = [repmat(temperature_data, [1, size(v_data,1)]); [v_data(1,:), v_data(2,:), v_data(3,:), v_data(4,:)]]'; v_temp_corr = corrcoef(v_temp);
...
%% Save the data:
save('Stochastic_model_updating_CaseII')
