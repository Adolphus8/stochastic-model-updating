%% Material characterisation [Part 1]:
% 
...
k_temp = [repmat(temperature_data, [1, 4]); [k_data(1,:), k_data(2,:), k_data(3,:), k_data(4,:)]]'; k_temp_corr = corrcoef(k_temp);
sprintf('The Pearson correlation coefficient between Temperature and Thermal conductivity is = %0.5g', k_temp_corr(1,2))

v_temp = [repmat(temperature_data, [1, 4]); [v_data(1,:), v_data(2,:), v_data(3,:), v_data(4,:)]]'; v_temp_corr = corrcoef(v_temp);
...
m = reg_coeff(2,1);
k_res = [[0.0496, 0.0530, 0.0493, 0.0455] - ((m*20) + reg_coeff(1,1)),...
         [0.0628, 0.0620, 0.0537, 0.0561] - ((m*250) + reg_coeff(1,1)), ...
         [0.0602, 0.0546, 0.0628, 0.0614] - ((m*500) + reg_coeff(1,1)), ...
         [0.0657, 0.0713, 0.0694, 0.0732] - ((m*750) + reg_coeff(1,1)), ...
         [0.0631, 0.0746, 0.0692, 0.0739] - ((m*1000) + reg_coeff(1,1))]'; 
...
xlabel('$T$ $[^oC]$', 'Interpreter', 'latex'); ylabel('$k$ $[W/{m} \cdot {^oC}]$', 'Interpreter', 'latex'); set(gca, 'Fontsize', f)
subplot(2,2,2)
hold on; box on; grid on;
for i = 1:size(v_data,1)
scatter(temperature_data, v_data(i,:), 30, 'blue', 'filled')
end
xlabel('$T$ $[^oC]$', 'Interpreter', 'latex'); ylabel('$\rho$ $C_p$ $[J/{m^3} \cdot {^oC}]$', 'Interpreter', 'latex'); set(gca, 'Fontsize', f); ylim([3.0e5, 5e5])
subplot(2,2,3)
hold on; box on; grid on;
histogram(k_res, 5);
xlabel('$\epsilon_k$ $[W/{m} \cdot {^oC}]$', 'Interpreter', 'latex'); ylabel('Count'); set(gca, 'Fontsize', f)
subplot(2,2,4)
hold on; box on; grid on;
histogram(v_data, 5);
xlabel('$\rho$ $C_p$ $[J/{m^3} \cdot {^oC}]$', 'Interpreter', 'latex'); ylabel('Count'); set(gca, 'Fontsize', f); xlim([3.0e5, 5e5])
...
% Update the hyper-parameters of the SDF for the Thermal conductivity residual variable:
priorpdf_k = @(x) isfeasible(bounds(1,:), x);  % Prior for Thermal conductivity hyper parameters;
priorrnd_k = @(N) prior_staircaseRV(N, 1, bounds(1,:)); 
width_k = 0.05;
logL_k = @(theta) loglikelihood(theta, k_res, bounds(1,:), dm, width_k, Nsim);  

tic;
TEMCMC1 = TEMCMCsampler('nsamples', N, 'loglikelihood', logL_k, 'priorpdf', priorpdf_k, 'priorrnd', priorrnd_k);
timeTEMCMC1 = toc;
sample_epsilon_k = TEMCMC1.samples;

% Update the hyper-parameters of the SDF for the Volumetric heat capacity variable:
...
a = 45; b = 47;
ci_epsilon_k = [prctile(sample_epsilon_k, [a, 100-a])]'; ci_rho = [prctile(sample_rho, [b, 100-b])]';
...
plot([max(x1),max(x2)],[1,1], 'b', 'linewidth', 2, 'handlevisibility', 'off'); xlim([-0.02, 0.02])
...
stairs(x1, f1, 'b', 'linewidth', 2); stairs(x2, f2, 'b', 'linewidth', 2, 'handlevisibility', 'off'); 
plot([min(x1),min(x2)],[0,0], 'b', 'linewidth', 2, 'handlevisibility', 'off'); 
plot([max(x1),max(x2)],[1,1], 'b', 'linewidth', 2, 'handlevisibility', 'off'); xlim([3e5, 5e5])
set(gca, 'Fontsize', f); xlabel('$\rho C_p$ $[J/{m^3} \cdot {^oC}]$', 'Interpreter', 'latex'); ylabel(ylab); 

%% Save the data:
save('Material_characterisation_Part1')
