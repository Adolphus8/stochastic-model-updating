...
temperature_data = [20, 500, 1000];

k_data = [0.0496, 0.0602, 0.0631; 0.0530, 0.0546, 0.0796];

v_data = [3.76E+05, 4.52E+05, 4.19E+05; 3.38E+05, 4.10E+05, 4.38E+05];

%% Pair the data to compute the Pearson correlation:
clc;

k_temp = [repmat(temperature_data, [1, size(k_data,1)]); [k_data(1,:), k_data(2,:)]]'; k_temp_corr = corrcoef(k_temp);
sprintf('The Pearson correlation coefficient between Temperature and Thermal conductivity is = %0.5g', k_temp_corr(1,2))

v_temp = [repmat(temperature_data, [1, size(v_data,1)]); [v_data(1,:), v_data(2,:)]]'; v_temp_corr = corrcoef(v_temp);
...
width_k = 0.007;
logL_k = @(theta) loglikelihood(theta, [k_res; k_res], bounds(1,:), dm, width_k, Nsim);  
...
width_rho = 0.005;
logL_rho = @(theta) loglikelihood(theta, [v_data(:); v_data(:)].*1e-07, bounds(2,:), dm, width_rho, Nsim); 
...
%% Save the data:
save('Stochastic_model_updating_CaseI')
