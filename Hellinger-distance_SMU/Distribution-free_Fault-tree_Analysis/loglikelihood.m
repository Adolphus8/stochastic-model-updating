...
%         - 5 for Jensen-Shannon divergence distance metric
%         - 6 for Hellinger distance metric
...
d_sim = SDF_rnd(theta, bounds, 2, Nsim); dist = d(d_sim, data);
logL(i) = - (dist./width).^2;
...
%%
% INPUT: 
% samp1 (model_samp): Nsim x dim matrix of data y_exp samples;
% samp2 (data_samp):  Ndata x dim matrix of simulated model output y_sim samples;
%
% OUTPUT: 
