...
# the operators %*% and %/% (which in R normally denote matrix multiplication and 
# integer division) have been overloaded, so they still do matrix multiplication and integer 
# division when both operands are precise.  If either operand is uncertain, however, they 
# denote the Frechet multiplication and division operations.  Also notice that there are no 
# autoselected infix operators for minimum and maximum. The pmin and pmax functions 
# will return the Frechet convolutions. Also notice that &, |, &&, || have not been overloaded 
# for uncertain numbers because R has sealed those operators. You must use the operators 
...
Pbox$lwd <- 3                      # thickness of lines in plotting
...
reciprocate.pbox <- function(x, doit=FALSE) {
  if (x@shape == 'chi-squared') return(inversechisquared(mean(x)))  # should go the other way too
  #if (x@shape == 'gamma') {m = mean(x); v = var(x); return(inversegamma(v/m^2, v/m))}  # should go the other way too
  #if (x@shape == 'beta') {m = mean(x); v = var(x); return(inversebeta())}  # should go the other way too
  if (doit) if (x@u[[1]]==0) x@u[[1]] = x@d[[1]] / 2  # try to invert even if it "touches" zero
  pairs=c('Pareto','power function','gamma','inverse gamma','chi-squared','inverse chi-squared','beta','inverse beta')
  if (x@shape %in% c('Cauchy','{min, max, median}','{min, max, percentile}','{min, max}')) sh <- x@shape else
  if (x@shape %in% pairs) {w = match(x@shape, pairs); if (w%%2==0) w=w-1 else w=w+1; sh=pairs[[w]]} else sh=''
  t = reciprocate; 
...
# The operator %/% is reassigned to Frechet division, which clobbers its use for integer division.  Use intdiv instead.
#intdiv = function(x,y) (x-(x %% y)) / y  # integer division (we redefine %/%)
...
#######################################################################
# Some quantile functions for distributions not already implemented in R #
#######################################################################
...
Sbetabinomial <- function(a,b,n) { # cf. betabinomial <- function(size, v, w, r=runif(MC$many)) mc(qbinom(r_(r), size, beta(v,w)@x)) 
  k <- 0:n
...
Sinversechisquared <- function(df, name=''){
  library("VGAMextra")
  if (df>2) ml=mh = 1/(df-2) else  {ml=0; mh=Inf}
  if (df>4) vl=vh = 2/((df-2)^2*(df-4)) else {vl=0; vh=Inf}
  pbox(u=qinv.chisq(ii(),df), d=qinv.chisq(jjj(),df), shape='inverse chi-squared', name=name, ml=ml, mh=mh, vl=vl, vh=vh)
  }
...
Sgamma <- function(scale=1/rate, shape, rate=1, name=''){    # N.B. the parameters are {scale=1/rate, shape, rate=1} which are different from rgamma's parameters which are {shape, rate=1, scale =1/rate}
  m <- scale*shape 
  v <- scale^2*shape 
  pbox(u=qgamma(ii(), shape=shape, scale=scale), d=qgamma(jjj(), shape=shape, scale=scale), shape='gamma', name=name, ml=m, mh=m, vl=v, vh=v) 
  } 

Sgamma1 <- function(mu, sigma, name='') return(Sgamma(scale=sigma^2/mu, shape=(mu/sigma)^2, name='')) 
...
Sinversegamma <- function(shape, rate = 1, scale = 1/rate, name=''){ 
  m <- scale/(shape-1)
  v <- scale^2/((shape-1)^2*(shape-2))
  pbox(u=rev(1/qgamma(jj(), shape=shape, scale=1/scale)), d=rev(1/qgamma(iii(), shape=shape, scale=1/scale)), shape='inverse gamma', name=name, ml=m, mh=m, vl=v, vh=v) 
  } 
...
# if (identical(dname,Sloguniform)) a@shape = 'loguniform' else a@shape = 'uniform'
# if (Pbox$plotting.every) try(plot.pbox(a))
# #a = pbox(a, ...)    # if I use this line, then the evaluation of U(interval(2,3), interval(1,4)) has a puffy variance [0, 0.79] rather than the correct value [0,0.333] 
# return(a)
# }
# 
## SU <- Suniform <- Srectangular <- function(min, max, name=''){
...
inversechisquared <- function(df, ...) env1.2(Sinversechisquared,df, ...)
...
gamma <- erlang <- function(scale=1/rate, shape, rate=1, ...) {env2.4(Sgamma,scale,shape, ...)}
gamma1 <- function(mu, sigma, ...) return(gamma(scale=sigma^2/mu, shape=(mu/sigma)^2, ...))
gamma2 <- function(shape, scale, ...) env2.4(Sgamma2,shape, scale, ...) 
inversegamma <- function(shape, rate = 1, scale = 1/rate, ...) env2.4(Sinversegamma,shape,scale, ...)

#gammaexponential <- function(shape, scale=1, rate=1/scale, ...) env2.4(Sgammaexponential, shape=shape, scale=scale, ...)
#
...
MMgamma <- function(x) {a = mean(x); b= sd(x); gamma(scale=b^2/a, shape=(a/b)^2)}  #gamma1(a, b) ~ gamma(b²/a, (a/b)²)
...
CBnormal.sigma <- function(x) {n <- length(x); return(sqrt(var(x)*(n-1)*inversechisquared(n-1)))}
...
CBlognormal.sigma <- function(x) {n <- length(x); return(sqrt(var(log(x))*(n-1)*inversechisquared(n-1)))}
...
FKN <- function(k,n) { # binomial rate inference for trials designed with a fixed-K stopping rule
  if ((left(k) < 0)  || (right(n) < right(k))) stop('Improper arguments to function KN')
  Bzero <- 1e-6
  Bone <- 1-Bzero
  return(pmin(pmax(env(Sbeta(left(k),right(n)-left(k)+1),Sbeta(right(k),pmax(0,left(n)-right(k)))),Bzero),Bone))
  }
...
# Wassertstein area metric 
#wam = function(r, d,a,b, many = 10000) {    # for 'exp', use b=1 
#  rr = rep(r, many/length(r)) # okay if it doesn�t fit perfectly
#  many = length(rr)  
#  bit = 1/(many-1)
#  p = seq(bit,1-bit,length.out=many)
#  x = do.call(paste('q',d,sep=''),list(p,a,b))	  
#  D = sum(abs(sort(rr) - x))/many
#  return(D)
#  }
wam = function(r, x=NULL, d=NULL,a=NULL,b=NULL, many = 100000) {  # for 'exp', use b=1 
  rr = rep(r, many/length(r)) # okay if it doesn�t fit perfectly
  if (missing(x)) {
    many = length(rr)
    bit = 1/(many-1)
  ...
  xx = rep(x, many/length(x))
  L = min(length(rr), length(xx))
  D = abs(sort(rr[1:L]) - sort(xx[1:L]))
  return(sum(D)/L)
}
#many=1000
#r = runif(4)
#wam(r,d='unif',a=0,b=1) # 0.1289052
#wam(r,x=runif(many))  # 0.1365437
#wam(r,x=runif(many))  # 0.1433417
#many = 10000
#wam(r,x=runif(many))  # 0.1262292
#wam(r,x=runif(many))  # 0.1278321
#wam(r,d='unif',a=0,b=1) # 0.1289052
...
psd = function(x) { # population standard deviation
  n = length(x)
  sqrt(((n-1)*sd(x)^2)/n)
  }

pvar = function(x) { # population variance
  n = length(x)
  (n-1)*var(x)/n
  }

resample <- function(x, ...) x[sample.int(length(x), ...)] # sample() misbehaves if length(x)=0

wait = function(msg="Press [enter] to continue") invisible(readline(prompt=msg))

onKeybd <- function(key) keyPressed <<- key

waitgraph <- function(prompt="Press any key to continue") {
  if (.Platform$OS.type=='windows') if(!RStudio) bringToTop(-1)
  getGraphicsEvent(prompt = prompt, onMouseDown = NULL, onMouseMove = NULL,
  onMouseUp = NULL, onKeybd = onKeybd, consolePrompt = prompt)
  Sys.sleep(0.01)
  key = keyPressed
  if (.Platform$OS.type=='windows') if(!RStudio) bringToTop(-1)
...
catn = function (..., file = "", sep = " ", fill = FALSE,  labels = NULL, append = FALSE) cat(...,'\n', file=file, sep=sep, fill=fill, labels=labels, append=append) # appends CRLF
...
black      = bla = function(x,lwd=Pbox$lwd,col='black',...)      edf(x,lwd=lwd,col=col,...)
red        = red = function(x,lwd=Pbox$lwd,col='red',...)        edf(x,lwd=lwd,col=col,...)
blue       = blu = function(x,lwd=Pbox$lwd,col='blue',...)       edf(x,lwd=lwd,col=col,...)
green      = gre = function(x,lwd=Pbox$lwd,col='green3',...)     edf(x,lwd=lwd,col=col,...)
khaki      = kha = function(x,lwd=Pbox$lwd,col='khaki2',...)     edf(x,lwd=lwd,col=col,...)
navy       = nav = function(x,lwd=Pbox$lwd,col='navy',...)       edf(x,lwd=lwd,col=col,...)
purple     = pur = function(x,lwd=Pbox$lwd,col='purple',...)     edf(x,lwd=lwd,col=col,...)
brown      = bro = function(x,lwd=Pbox$lwd,col='brown4',...)     edf(x,lwd=lwd,col=col,...)
olive      = oli = function(x,lwd=Pbox$lwd,col='olivedrab4',...) edf(x,lwd=lwd,col=col,...) 
gray       = gra = function(x,lwd=Pbox$lwd,col='gray',...)       edf(x,lwd=lwd,col=col,...)
orange     = ora = function(x,lwd=Pbox$lwd,col='darkorange',...) edf(x,lwd=lwd,col=col,...)
pink       = pin = function(x,lwd=Pbox$lwd,col='deeppink',...)   edf(x,lwd=lwd,col=col,...)
cyan       = cya = function(x,lwd=Pbox$lwd,col='cyan',...)       edf(x,lwd=lwd,col=col,...)
chartreuse = cha = function(x,lwd=Pbox$lwd,col='chartreuse',...) edf(x,lwd=lwd,col=col,...)
gold       = gol = function(x,lwd=Pbox$lwd,col='gold',...)       edf(x,lwd=lwd,col=col,...)
ecru       = ecr = function(x,lwd=Pbox$lwd,col='tan',...)        edf(x,lwd=lwd,col=col,...) # 'tan' would conflict with tangent
brick      = bri = function(x,lwd=Pbox$lwd,col='firebrick3',...) edf(x,lwd=lwd,col=col,...)
sienna     = sie = function(x,lwd=Pbox$lwd,col='sienna',...)     edf(x,lwd=lwd,col=col,...)
rust       = rus = function(x,lwd=Pbox$lwd,col='sienna3',...)     edf(x,lwd=lwd,col=col,...)
salmon     = sal = function(x,lwd=Pbox$lwd,col='pink1',...)   edf(x,lwd=lwd,col=col,...)
magenta  = mag = function(x,lwd=Pbox$lwd,col='magenta',...)   edf(x,lwd=lwd,col=col,...)
yellow     = yel = function(x,lwd=Pbox$lwd,col='gold',...)       edf(x,lwd=lwd,col=col,...)
white      = whi = function(x,lwd=Pbox$lwd,col='white',...) edf(x,lwd=lwd,col=col,...)
teal       = teal = function(x,lwd=Pbox$lwd,col=rgb(0,128/255,128/255),...)       edf(x,lwd=lwd,col=col,...)

# this scheme (below) seems to work in sra.r to show the variable name, but may be using a different edf as it seems to fail with "new=TRUE"
...
# gamma and inversegamma
par(mfrow=c(2,2))
many = 20000
shape= 2
scale= 3
rate = 1/scale
A = rgamma(many, shape=shape, scale=scale)
B = gamma(shape=shape,scale=scale)
blue(A,new=TRUE);  edf(B,col='green')  # should be identical
A = rgamma(many, shape=shape, rate=rate)
B = gamma(shape=shape,scale=scale)
blue(A,new=TRUE);  edf(B,col='green')  # should be identical
a = gamma(shape=shape,scale=scale)
b = rgamma(many,shape=shape,scale=scale)
c = inversegamma(shape,1/scale)
d = 1/b
blue(a,new=TRUE);  edf(b,col='green')  # should be identical
blue(c,new=TRUE);  edf(d,col='green')  # should be identical
...
scale=interval(4,5)
rate = 1/scale
A1 = rgamma(many, shape=left(shape), scale=left(scale))
A2 = rgamma(many, shape=left(shape), scale=right(scale))
A3 = rgamma(many, shape=right(shape), scale=left(scale))
A4 = rgamma(many, shape=right(shape), scale=right(scale))
B = gamma(shape=shape,scale=scale)
blue(A1,new=TRUE,xlim=range(c(A1,A2,A3,A4))); blue(A2); blue(A3); blue(A4)
edf(B,col='green')  # should envelope
A1 = rgamma(many, shape=left(shape), rate=left(rate))
A2 = rgamma(many, shape=left(shape), rate=right(rate))
A3 = rgamma(many, shape=right(shape), rate=left(rate))
A4 = rgamma(many, shape=right(shape), rate=right(rate))
B = gamma(shape=shape,scale=scale)
blue(A1,new=TRUE,xlim=range(c(A1,A2,A3,A4))); blue(A2); blue(A3); blue(A4)
edf(B,col='green')  # should envelope
A1 = rgamma(many, shape=left(shape), scale=left(scale))
...
edf(B,col='green')  # should envelope
C = inversegamma(shape,1/scale)
D1 = 1/A1; D2 = 1/A2; D3 = 1/A3; D4 = 1/A4; 
blue(D1,new=TRUE,xlim=range(c(D1,D2,D3,D4))); blue(D2); blue(D3); blue(D4)
edf(C,col='green')  # should envelope

par(mfrow=c(2,3))
scale = 2
shape = 3
rate = 1/scale
A = gamma(scale=scale,shape=shape)
B = gamma(scale,shape)
m = shape * scale;
s = sqrt(shape) * scale
C = gamma1(m, s)
D = gamma2(shape,scale)
E = inversegamma(shape,rate)
E@u[1] = E@d[1] / 2
E = reciprocate(E)
red(A,new=TRUE)
blue(B,new=TRUE)
...
black(D,new=TRUE)
gray(E,new=TRUE)
red(A,new=TRUE); blue(B); green(C); black(D); gray(E)
w = 0.01
for (i  in 1:100) {red(A); Sys.sleep(w); blue(B); Sys.sleep(w); green(C); Sys.sleep(w); black(D); Sys.sleep(w); gray(E)}

scale = 20
shape = 3
g = gamma(scale,shape)
h = reciprocate(g, doit=TRUE)
r = rgamma(many,scale=scale,shape=shape)
d = 1/r
i = inversegamma(shape,1/scale)
...
# chi-square and inverse chi-square
df = 20
g = chisquared(df)
r = rchisq(many,df)
h = reciprocate(g, doit=TRUE)
d = 1/r
i = inversechisquared(df)
green(g,new=TRUE)
edf(r)
red(i,new=TRUE)
edf(d)
...
for (n in c(5,15,500)) {
  ...
  m = CBnormal.mu(x)
  s = CBnormal.sigma(x)
  edf(m,new=TRUE); title('normal mean')
  edf(s,new=TRUE); title('normal standard deviation')
  cyan(N(5,1),new=TRUE); title('prediction, data, true (blue)')
  green(p)
  edf(x)
  }
return(TRUE)
