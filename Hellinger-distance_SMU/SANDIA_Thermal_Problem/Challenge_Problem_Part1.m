%% Part 1: Material characterisation - Stochastic model updating:
% 
...
width_k = 0.10;
logL_k = @(theta) loglikelihood(theta, k_res, bounds(1,:), width_k, Nsim);  
...
width_rho = 0.050;
logL_rho = @(theta) loglikelihood(theta, v_data(:).*1e-07, bounds(2,:), width_rho, Nsim); 
...
% Plot the PDF via KDE:
Nplot = 10000;
xin = [linspace(-0.01, 0.005, Nplot); linspace(0, 0.0002, Nplot); linspace(-3, 2.5, Nplot); linspace(0, 10, Nplot);... 
       linspace(360000, 430000, Nplot); linspace(0, 6.2e+09, Nplot); linspace(-2, 2, Nplot); linspace(0, 6.5, Nplot)];
...
subplot(2,4,i)    
hold on; box on; grid on;
plot(xin(i,:), f(i,:,1), 'b', 'linewidth', 2, 'handlevisibility', 'off'); set(gca, 'Fontsize', fz);
xline(map(i,1), 'k --', 'linewidth', 2); xline(mean_vec(i,1), 'r --', 'linewidth', 2);
xlabel(label{1,i}, 'Interpreter', 'latex'); ylabel('Normalised PDF value'); xlim([min(xin(i,:)), max(xin(i,:))]);

subplot(2,4,i+4)    
hold on; box on; grid on;
plot(xin(i+4,:), f(i,:,2), 'b', 'linewidth', 2, 'handlevisibility', 'off'); set(gca, 'Fontsize', fz);
xline(map(i,2), 'k --', 'linewidth', 2); xline(mean_vec(i,2), 'r --', 'linewidth', 2);
xlabel(label{2,i}, 'Interpreter', 'latex'); ylabel('Normalised PDF value'); xlim([min(xin(i+4,:)), max(xin(i+4,:))]);
end
legend('Maximum A-posteriori', 'Mean', 'linewidth', 2)
...
%% Construct the SDF distributions:
...
%% Save the data:
save('Challenge_Problem_Part1_Hellinger')
